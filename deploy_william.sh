...
for vm in "${cluster[@]}"; do
	...
	if [ $? -eq 0 ]; then
		sshpass -f $passwd_file rsync -avu -e "ssh -o 'StrictHostKeyChecking no'" start_william.sh $netid@$vm:~/start.sh
		# sshpass -f $passwd_file rsync -avu -e "ssh -o 'StrictHostKeyChecking no'" ssh_config $netid@$vm:~/ssh_config
		# sshpass -f $passwd_file rsync -avu -e "ssh -o 'StrictHostKeyChecking no'" ~/.ssh/cs425 $netid@$vm:~/ssh_cs425
		# sshpass -f $passwd_file rsync -avu -e "ssh -o 'StrictHostKeyChecking no'" ~/.ssh/cs425.pub $netid@$vm:~/ssh_cs425.pub
